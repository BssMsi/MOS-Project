...
class PlotGraph:
    ...
    def __init__(self):
        ...
        Label(header, text="Micro-Tensile Testing Machine", font=("Helvetica", 40, "bold")).grid(row=0)
        ...
        self.choice = IntVar()
        self.choice.set(1)
        Radiobutton(par_frame, text="Elongation", variable=self.choice, value=1).grid(row=0, column=0, padx=50)
        Radiobutton(par_frame, text="Bending", variable=self.choice, value=2).grid(row=1, column=0, padx=50)

        Label(par_frame, text="LENGTH(mm)", relief=SUNKEN).grid(row=0, column=1, ipadx=5, ipady=1)
        self.L = Entry(par_frame)
        self.L.insert(0, 25.4)
        self.L.grid(row=0, column=2)
        Label(par_frame, text="Area(mm^2)", relief=SUNKEN).grid(row=1, column=1, ipadx=5, ipady=1)
        self.area_box = Entry(par_frame)
        ...
        self.lines = live_plot.plot([0.0], [0.0], 'k-', [0.0], [0.0], 'r--')
        ...
        Label(label_frame, text="DATA\nS.No  Load(N)  Elongation(mm)\n",
              font=("Helvetica", 17, "bold")).grid(row=0, column=0, sticky=N + S + W + E)
        self.data = StringVar()
        self.data_str = ""
        self.data.set(self.data_str)
        Label(label_frame, textvariable=self.data, font=16).grid(row=1, column=0, sticky=N + S + W + E)
    ...
    def run_program(self):
        ...
        self.arrow1 = self.curve_plot.annotate('', xy=(self.strain[ind], self.stress[ind]),
                                               ...
                                               arrowprops=dict(arrowstyle="<->"))
        self.label1 = self.curve_plot.text(self.strain[ind] + 0.001, self.stress[ind] / 2, 'Tensile Strength')
    ...
    def update_plot(self, load, delta_l, L, x_shift, y_shift):
        # Plot graph
        if self.no_reading > 1:
            print (load - (self.stress[self.no_reading]) / (delta_l - self.strain[self.no_reading])), "-", load
